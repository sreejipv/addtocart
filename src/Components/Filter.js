...
function handleLeftrange(e){
    ...
    const newMarginLeft = `calc(${leftvalue}%)`
    ...
    const newPos = `calc(${leftvalue}% + (${8 -(leftvalue * 0.750) - gapValue}px))`
...
function setFilter() {
  setRange({left: leftvalue *100, right: rightvalue *100})
}
    ...
    return(
      <div className="filterSlider">
        ...
        <div className="multi-range pt-10">
                  <input ref={textInput1} className="filter lower" type="range" min={min} max={max} value={leftvalue} onChange={ handleLeftrange } />
                  <input ref={textInput2} className="filter upper" type="range" min={min} max={max} value={rightvalue} onChange={ handleRightrange} />
                  <div className="rangeSlider">
                    <div ref={filterValueLeft}  style={styledValueLeft} className="rangeSlider--range"></div>
                  </div>
                  <span ref={filterValueStart}  style={styledValueStart} className="startPoint">{leftvalue *100}</span>
                  <span ref={filterValueEnd} style={styledValueEnd}  className="endPoint">{rightvalue *100}</span>
          </div>
